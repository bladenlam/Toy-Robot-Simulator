...
let OutputArea = document.querySelector('#OutputArea') as HTMLHeadingElement;
...
//PLACE
function PLACE(X:number,Y:number,Direction:string): void
{
...
function LEFT()
{
    ...
    Toy.Orientation--;
    if (Toy.Orientation <= -1) {Toy.Orientation = 3};
}
//RIGHT
function RIGHT()
{
    ...
    Toy.Orientation++;
    if (Toy.Orientation >= 4) {Toy.Orientation = 0};
}
//REPORT
function REPORT()
{
...
//Submit Button Ref
const SubmitButton = document.querySelector('#ExecuteCommand') as HTMLButtonElement;
